...
def initialize_model(model_config, model_path):
...
def simulate_playoff_series(matchups, round_num):
    ...
    for matchup_key, matchup_info in matchups.items():
        ...
        for game_num in range(1, 8):
            ...
            # use Beta distribution to introduce more variance while keeping the same expected value
            # the shape parameters (alpha and beta) control the variance - lower values = higher variance

            '''
            alpha = win_probability * 3  # reduced multiplier for higher variance
            beta = (1 - win_probability) * 3  # reduced multiplier for higher variance
            '''
            # Apply home boost
            team_a_home_games = {1, 2, 5, 7}
            is_team_a_home = game_num in team_a_home_games
            home_boost = 0.05

            if is_team_a_home:
                adjusted_win_prob = win_probability + home_boost
            else:
                adjusted_win_prob = win_probability - home_boost
            
            # Clip to prevent extreme values
            adjusted_win_prob = np.clip(adjusted_win_prob, 0.01, 0.99)

            alpha = max(adjusted_win_prob * 0.5, 0.01)
            beta = max((1 - adjusted_win_prob) * 0.5, 0.01)
            ...
            noise_factor = np.random.normal(1.0, 0.5)
...
def simulate_tournament(model=None, model2 = None, threshold = 0.505):
    ...
    if model is None or model2 is None:
        ...
        model2_config = load_model_config("nba_tabnet_model_2_config.json")
        model = initialize_model(model_config, "NBA_TABNET_TEST_MODEL.zip")
        model2 = initialize_model(model2_config, "NBA_TABNET_TEST_MODEL_2.zip")
    ...
    while current_round <= max_rounds:
        ...
        flipped_stats = -current_stats.values
        probs1 = model.predict_proba(current_stats.values)
        probs2 = model2.predict_proba(current_stats.values)
        blended_forward = (probs1 + probs2) / 2

        flipped_probs1 = model.predict_proba(flipped_stats)
        flipped_probs2 = model2.predict_proba(flipped_stats)
        ...
        # Flipped input predictions (Team B - Team A)
        flipped_stats = -current_stats.values
        probs1_flipped = model.predict_proba(flipped_stats)
        probs2_flipped = model2.predict_proba(flipped_stats)
        blended_flipped = (probs1_flipped + probs2_flipped) / 2
        ...
        '''
        # 1) average the two models on the raw ordering
        blended_regular = (probs1 + probs2) / 2

        # 2) average the two models on the flipped ordering
        blended_flipped = (probs1_flipped + probs2_flipped) / 2

        # 3) extract Team-A‘s “class 1” probability in each case
        p_reg   = blended_regular[:,   1]
        p_flip  = 1 - blended_flipped[:, 1]

        # 4) symmetric blend
        final_probs = (p_reg + p_flip) / 2

        # 5) reconstruct the full two-column array
        probabilities = np.stack([1 - final_probs, final_probs], axis=1)
        positive_probs = final_probs
        '''

        '''
        else:
            predictions = model.predict(current_stats.values)
            probabilities = model.predict_proba(current_stats.values)
        '''
    ...
    try:
        ...
        for i in range(1, 100):
            try:
                ...
                msg = simulate_tournament(model=clf1, model2=clf2, threshold = 0.505)
                ...
                if os.path.exists(simulation_path):
                    ...
                    try:
                        ...
                        master_path = r"data\simulation_data_master.csv"
                        current_df = pd.read_csv(dest_file)

                        if os.path.exists(master_path):
                            current_df.to_csv(master_path, mode='a', header=False, index=False)
                        else:
                            current_df.to_csv(master_path, index=False)
